...
/**
 * @brief Change duration of each note.
 * 
 * This function makes the board output a tone/beep on pin PC3 by rapidly switching the pin 
 * between LOW and HIGH for a requested frequency and duration, which is a standard way to 
 * drive a buzzer or speaker from a GPIO pin.
 * 
 * @param freq The sound frequency in Hz.
 * @param dur The sound length in milliseconds.
 * 
**/
extern void JOY_sound(uint16_t freq, uint16_t dur);

/**
 * @brief Change duration of each note.
 * @param duration The duration of notes to be played.
**/
extern int convertDuration(int duration);
 ...
 * 
**/
extern inline void playAllMusic(void);
