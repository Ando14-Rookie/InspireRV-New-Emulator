...
uint16_t ADC_read(void) { return GPIO_analogRead(GPIO_Ain7_D4); }

uint16_t ADC_read_pad(void) { return GPIO_analogRead(GPIO_Ain2_C4); }

uint16_t ADC_read_smallboard(void) { return GPIO_analogRead(GPIO_Ain2_C4); }
...
#define JOY_act_released()                                                               \
    (JOY_check_button(multiple_ADC_reads(ADC_read_smallboard, 5)) != JOY_ACT)
#define JOY_up_pressed()                                                                 \
    (JOY_check_button(multiple_ADC_reads(ADC_read_smallboard, 5)) == JOY_UP)
#define JOY_down_pressed()                                                               \
    (JOY_check_button(multiple_ADC_reads(ADC_read_smallboard, 5)) == JOY_DOWN)
#define JOY_left_pressed()                                                               \
    (JOY_check_button(multiple_ADC_reads(ADC_read_smallboard, 5)) == JOY_LEFT)
#define JOY_right_pressed()                                                              \
    (JOY_check_button(multiple_ADC_reads(ADC_read_smallboard, 5)) == JOY_RIGHT)
#define JOY_X_pressed()                                                                  \
    (JOY_check_button(multiple_ADC_reads(ADC_read_smallboard, 5)) == JOY_X)
#define JOY_Y_pressed()                                                                  \
    (JOY_check_button(multiple_ADC_reads(ADC_read_smallboard, 5)) == JOY_Y)
...
static inline uint8_t JOY_up_pressed(void) {
    // Read the current analog value from the joypad.
    uint16_t val = ADC_read_pad();
    // Check if the value is within the deviation range of the calibrated value for up direction
    print("Value ADC: %d", val);
    /* Find calibrated ADC for up, up-left, up-right, 
    and check if the current value is within deviation range of any of them. */
...
static inline uint8_t JOY_down_pressed(void) {
    // Read the current analog value from the joypad.
    uint16_t val = ADC_read_pad();
    // Check if the value is within the deviation range of the calibrated value for down direction
    print("Value ADC: %d", val);
    /* Find calibrated ADC for down, down-left, down-right, 
    and check if the current value is within deviation range of any of them. */
...
static inline uint8_t JOY_left_pressed(void) {
    // Read the current analog value from the joypad.
    uint16_t val = ADC_read_pad();
    // Check if the value is within the deviation range of the calibrated value for left direction
    print("Value ADC: %d", val);
    /* Find calibrated ADC for up, up-left, up-right, 
    and check if the current value is within deviation range of any of them. */
...
static inline uint8_t JOY_right_pressed(void) {
    // Read the current analog value from the joypad.
    uint16_t val = ADC_read_pad();
    // Check if the value is within the deviation range of the calibrated value for right direction
    print("Value ADC: %d", val);
    /* Find calibrated ADC for up, up-left, up-right, 
    and check if the current value is within deviation range of any of them. */
