...
color_t paintSaveSlots[SAVE_SLOTS][NUM_LEDS];
bool paintSlotUsed[SAVE_SLOTS] = {false};
...
void saveOrLoadPaintToSlot(void) {
    ...
    for (int i = 0; i < NUM_LEDS; i++) {
        paintSaveSlots[slotIndex][i] = savedColor[i];
    }
}
...
void renderSaveLoadSlotScreen(void) {
    ...
    while (!slotChosen) {
        ...
        if ((currentPage == PAINTING_SAVE_SLOT)) {
            // Ask slot from user
            slotIndex = userSlotIndex();
            printf(
                "Slot %d selected, press Enter to confirm or 9 to go back.\n", slotIndex);
            // Change pageState to PAINTING_SAVE_CONFIRM
            currentPage = PAINTING_SAVE_CONFIRM;
        }
        else if (currentPage == PAINTING_LOAD_SLOT) {
            // Ask slot from user
            slotIndex = userSlotIndex();
            printf(
                "Slot %d selected, press Enter to confirm or 9 to go back.\n", slotIndex);
            // Change pageState to PAINTING_SAVE_CONFIRM
            currentPage = PAINTING_LOAD_CONFIRM;
        }
        // 2nd enter: confirm then go back to original savedColor canvas
        else if ((currentPage == PAINTING_SAVE_CONFIRM) ||
                 (currentPage == PAINTING_LOAD_CONFIRM)) {
            ...
            if (printOnce == 0) {
                ...
                for (int arrayRow = 0; arrayRow < 8; arrayRow++) {
                    ...
                    for (int arrayCol = 0; arrayCol < 8; arrayCol++) {
                        ...
                        switch (currentPage) {
                            case PAINTING_SAVE_CONFIRM:
                                val = saveConfirmLogo[slotIndex][arrayRow][arrayCol];
                                break;
                            case PAINTING_LOAD_CONFIRM:
            ...
            if (BTN_JUST_PRESSED(BTN_ENTER)) {
                ...
                if (currentPage == PAINTING_SAVE_CONFIRM) {
                    for (int i = 0; i < NUM_LEDS; i++) {
                        paintSaveSlots[slotIndex][i] = savedColor[i];
                    }
                ...
                else if (currentPage == PAINTING_LOAD_CONFIRM) {
                    for (int i = 0; i < NUM_LEDS; i++) {
                        savedColor[i] = paintSaveSlots[slotIndex][i];
                    }
